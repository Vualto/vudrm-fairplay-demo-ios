...
class AssetPersistenceManager: NSObject {
    ...
    func downloadStream(for asset: Asset) {
        ...
        guard let task =
            assetDownloadURLSession.aggregateAssetDownloadTask(with: asset.urlAsset,
                                                               ...
                                                               assetTitle: asset.stream.contentID,
        ...
        task.taskDescription = asset.stream.contentID
        ...
        userInfo[Asset.Keys.name] = asset.stream.contentID
    ...
    func assetForStream(withName name: String) -> Asset? {
        ...
        for (_, assetValue) in activeDownloadsMap where name == assetValue.stream.contentID {
            asset = assetValue
            break
        }
    ...
    func downloadState(for asset: Asset) -> Asset.DownloadState {
        ...
        if let localFileLocation = localAssetForStream(withName: asset.stream.contentID)?.urlAsset.url {
        ...
        for (_, assetValue) in activeDownloadsMap where asset.stream.contentID == assetValue.stream.contentID {
            return .downloading
        }
    ...
    func deleteAsset(_ asset: Asset) {
        ...
        do {
            if let localFileLocation = localAssetForStream(withName: asset.stream.contentID)?.urlAsset.url {
                ...
                userDefaults.removeObject(forKey: asset.stream.contentID)
...
extension AssetPersistenceManager: AVAssetDownloadDelegate {
    ...
    func urlSession(_ session: URLSession, task: URLSessionTask, didCompleteWithError error: Error?) {
        ...
        if let error = error as NSError? {
            ...
            case (NSURLErrorDomain, NSURLErrorCancelled):
                 ...
                 */
                guard let localFileLocation = localAssetForStream(withName: asset.stream.contentID)?.urlAsset.url else { return }
                ...
                do {
                    ...
                    print("An error occured trying to delete the contents on disk for \(asset.stream.contentID): \(error)")
                }
            ...
            do {
                ...
                userDefaults.set(bookmark, forKey: asset.stream.contentID)
            } catch {
    ...
    func urlSession(_ session: URLSession, aggregateAssetDownloadTask: AVAggregateAssetDownloadTask,
        ...
        aggregateAssetDownloadTask.taskDescription = asset.stream.contentID
